...
public:
	...
	void WriteToFile(const string filename) {
	    const unsigned int shape[] = {(unsigned int)var.rows(), (unsigned int)var.cols()};
	...
	void ReadFromFile(const string filename) {
		...
		int itr = 0;
		var = T::Zero(arr.shape[0], arr.shape[1]);
		for(int i = 0; i < arr.shape[0]; i++) {
			for(int j = 0; j < arr.shape[1]; j++) {
				var(i, j) = loaded_data[itr++];
			}
		}
	}
...
#pragma omp critical
		...
		//model.WriteVectorsToFile(out_file);
	}
...
int main(int argc, char **argv) {
	...
	if (argc == 7) {
		...
		Train(outfilename, factor, num_cores, l1_reg, l2_reg, word_vecs);
	} else {
